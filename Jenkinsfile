                ...
                withAWS(credentials: 'aws-eks-creds', region: "${AWS_REGION}") {
                    script {
                        sh """
                            ...
                            kubectl rollout status deployment/tindibandi-app --timeout=300s
                ...
                withAWS(credentials: 'aws-eks-creds', region: "${AWS_REGION}") {
                    script {
                        sh """
                            ...
                            i=1
                            while [ \$i -le 10 ]; do
                                EXTERNAL_IP=\$(kubectl get service tindibandi-service -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
                                EXTERNAL_HOSTNAME=\$(kubectl get service tindibandi-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null || echo "")
